...
export default function Carousel({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="relative w-full overflow-hidden px-8 md:px-12 py-4">
            <div 
            ...
            <div className="absolute z-30 flex -translate-x-1/2 bottom-0 left-1/2 space-x-3 rtl:space-x-reverse">
                {items.map((_, index) => (
                    <button
                        key={index}
                        type="button"
                        className={`w-3 h-3 rounded-full transition-all ${index === currentIndex ? "bg-(--text-primary)" : "bg-(--text-secondary) opacity-50"}`}
